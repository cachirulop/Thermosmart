...
public class CircularSeekBar extends View {
    ...
    private static final float DEFAULT_CURRENT_VALUE = 25;
    private static final int DEFAULT_CURRENT_VALUE_COLOR = Color.argb(235, 255, 138, 13);
    private static final float DEFAULT_CURRENT_VALUE_STROKE_WIDTH = 10;
    ...
    private float mCurrentValue = DEFAULT_CURRENT_VALUE;
    private int mCurrentValueColor = DEFAULT_CURRENT_VALUE_COLOR;
    private float mCurrentValueStrokeWidth = DEFAULT_CURRENT_VALUE_STROKE_WIDTH;
    private Paint mCurrentValuePaint;
    private float mCurrentValuePosition;
    private Path mCurrentValuePath;
    private float mCurrentValueDegrees;
    ...
    private void initPaints() {
        ...
        mCurrentValuePaint.setDither(true);
        mCurrentValuePaint.setColor(mCurrentValueColor);
        mCurrentValuePaint.setStrokeWidth(mCurrentValueStrokeWidth);
        mCurrentValuePaint.setStyle(Paint.Style.STROKE);
        mCurrentValuePaint.setStrokeJoin(Paint.Join.ROUND);
    }
    ...
    private void calculateCurrentValuePosition() {
        ...
        float currentValueDegrees = (currentValuePercent * mTotalCircleDegrees);
        mCurrentValuePosition = mStartAngle + (mIsInNegativeHalf ? -currentValueDegrees : currentValueDegrees);
        mCurrentValuePosition = (mCurrentValuePosition < 0 ? 360f + mCurrentValuePosition : mCurrentValuePosition) % 360f;
    }

    private void calculateCurrentValueDegrees() {
        mCurrentValueDegrees = mIsInNegativeHalf ? mStartAngle - mCurrentValuePosition : mCurrentValuePosition - mStartAngle; // Verified
        mCurrentValueDegrees = (mCurrentValueDegrees < 0 ? 360f + mCurrentValueDegrees : mCurrentValueDegrees); // Verified
    }
    ...
    private void initPaths() {
        ...
        mCirclePointerPath = new Path();
        mCurrentValuePath = new Path();
    }
    ...
    private void resetPaths() {
        if (mIsInNegativeHalf) {
            ...
            mCurrentValuePath.reset ();
            mCurrentValuePath.addArc (mCircleRectF, extendStart, mCurrentValueDegrees);

            float pointerStart = mPointerPosition - mPointerAngle / 2.0f;
            mCirclePointerPath.reset ();
            mCirclePointerPath.addArc (mCircleRectF, pointerStart, mPointerAngle);
        } else {
            mCirclePath.reset();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Current value
        canvas.drawPath (mCurrentValuePath, mCurrentValuePaint);
    }
...
/*
        float currentValue = mMax * mCurrentValue / mTotalCircleDegrees;
        return mIsInNegativeHalf ? -currentValue : currentValue;
